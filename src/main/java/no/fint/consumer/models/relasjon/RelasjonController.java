...
@CrossOrigin
@RestController
@RequestMapping(value = RestEndpoints.RELASJON, produces = {FintRelationsMediaType.APPLICATION_HAL_JSON_VALUE, MediaType.APPLICATION_JSON_UTF8_VALUE})
public class RelasjonController {
    ...
    @Autowired
    private RelasjonAssembler assembler;
    ...
    @GetMapping
    public ResponseEntity getRelasjon(
        ...
        List<FintResource<Relasjon>> relasjon;
        ...
        return assembler.resources(relasjon);
    }
    ...
    public ResponseEntity getRelasjonById(@PathVariable String id,
        ...
        Optional<FintResource<Relasjon>> relasjon = cacheService.getRelasjonById(orgId, id);

        fintAuditService.audit(event, Status.CACHE_RESPONSE, Status.SENT_TO_CLIENT);

        if (relasjon.isPresent()) {
            return assembler.resource(relasjon.get());
        } else {
